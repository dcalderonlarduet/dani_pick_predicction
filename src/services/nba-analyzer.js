...
import { buildNbaGameContext, loadNbaScoreboard } from "../providers/espn-nba.js";
import { getApiSportsBasketballConfig } from "../providers/api-sports-basketball.js";
import {
  loadNbaDroppingOdds,
  loadNbaOddsMapForDates,
  loadNbaValueBets,
  oddsApiSkipMarketSignals,
} from "../providers/odds-api-io.js";
import { shiftDateString } from "../utils/madrid-date.js";
import { isProGameInSlateWindow, isProGameOnTargetDate, isProRecommendationBettable } from "../utils/bettable-events.js";
...
import { evaluateNbaGamePicks } from "./nba-odds-policy.js";
...
async function buildNbaGame({ teams, scheduleDate, oddsMap, vbIndex, dropIndex, scoreboardCache }) {
  ...
  if (context?.flags) {
    ...
    context.flags.freshness_ok = Boolean(
      context.flags.stats_espn_disponibles &&
  ...
  game.recommendations = await evaluateNbaGamePicks(game);
  game.picks = game.recommendations.filter((pick) => pick.bettable);
  return game;
}

export async function loadNBASlate(date) {
  const events = await loadNbaScoreboard(date);
  return events
    .map((event) => {
...
export async function buildNbaAnalysis(date = todayInMadrid()) {
  const oddsRuntime = resolveOddsConfig();
  const apiSportsEnabled = getApiSportsBasketballConfig().enabled;
  ...
  const [eventsYesterday, eventsToday, eventsTomorrow, oddsMap, vbBet365, vbWinamax, dropping] =
    await Promise.all([
      loadNbaScoreboard(yesterday).catch(() => []),
      loadNbaScoreboard(date).catch(() => []),
      loadNbaScoreboard(tomorrow).catch(() => []),
      loadNbaOddsMapForDates(slateDates, oddsRuntime).catch(() => new Map()),
      skipSignals ? Promise.resolve([]) : loadNbaValueBets("Bet365").catch(() => []),
      skipSignals ? Promise.resolve([]) : loadNbaValueBets("Winamax FR").catch(() => []),
      skipSignals ? Promise.resolve([]) : loadNbaDroppingOdds(5, "12h").catch(() => []),
    ]);
  ...
  if (!raw.length) {
    return buildProUnavailableAnalysis("nba", date, "No hay partidos NBA en ventana ayer/hoy/mañana.", {
  ...
  const builtGames = await mapWithConcurrency(raw, 4, (entry) =>
    buildNbaGame({ ...entry, oddsMap, vbIndex, dropIndex, scoreboardCache })
  );
  ...
  const picks = bettable
    .flatMap((game) => (game.picks || []).map((pick) => mapNbaPickToUi(pick, game)))
    .sort((a, b) => (b.score || 0) - (a.score || 0))
    .slice(0, 24);
  const modelPicks = upcoming
    .flatMap((game) => (game.recommendations || []).map((pick) => mapNbaPickToUi(pick, game)))
    .sort((a, b) => (b.score || 0) - (a.score || 0))
    .slice(0, 40);
...
function formatNbaPickLabel(pick, game) {
  ...
  const market = NBA_MARKET_LABELS[pick.market] || pick.market;
  return pick.line != null ? `${market} · ${sideLabel} ${pick.line}` : `${market} · ${sideLabel}`;
}

function mapNbaPickToUi(pick, game) {
  ...
  return {
    ...
    pick: formatNbaPickLabel(pick, game),
    mercado: NBA_MARKET_LABELS[pick.market] || pick.market,
    market: pick.market,
    betSide: pick.side,
    linea: pick.line,
    line: pick.line,
