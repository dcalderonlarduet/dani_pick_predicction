// Browser-adapted copy of src/utils/madrid-date.js.
// Timezone is hardcoded (no process.env in browsers). Keep logic in sync with backend.
const APP_TIMEZONE = "Europe/Madrid";
const DATE_RE = /^\d{4}-\d{2}-\d{2}$/;

export function getDateStringInTimezone(date = new Date(), timeZone = APP_TIMEZONE) {
...
export function getMadridTodayDateString(now = new Date()) {
  return getDateStringInTimezone(now, APP_TIMEZONE);
}
...
export function isSameCalendarDayInTimezone(isoValue, targetDate, timeZone = APP_TIMEZONE) {
...
export function isMatchInAnalysisWindow(isoValue, targetDate, timeZone = APP_TIMEZONE) {
...
export function diffCalendarDaysInTimezone(isoValue, targetDate, timeZone = APP_TIMEZONE) {
...
export function isDateWithinCalendarWindow(
  isoValue,
  targetDate,
  { pastDays = 0, futureDays = 0, timeZone = APP_TIMEZONE } = {}
) {
